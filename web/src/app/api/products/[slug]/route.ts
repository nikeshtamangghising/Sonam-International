...
// Mock data for development (same as in the products route)
const mockProducts = [
  {
    id: 'product-1',
    sku: 'WS-T-001',
...
export async function PUT(
  ...
  try {
    ...
    // Find product by slug
    const productIndex = mockProducts.findIndex(p => p.slug === slug);
    ...
    return NextResponse.json(
      { message: 'Product updated successfully', product: { ...mockProducts[productIndex], ...body } }
    );
  } catch (error) {
